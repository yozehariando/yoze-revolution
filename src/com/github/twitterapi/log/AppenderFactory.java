package com.github.twitterapi.log;

import java.util.Enumeration;
import java.util.Hashtable;

import com.github.twitterapi.util.properties.Properties;
import com.github.twitterapi.util.string.StringUtils;
...
public class AppenderFactory {
	...
	private static Appender GOD_APPENDER = createAppender("GOD",
			Appender.CONSOLE, "DEBUG", null);
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				if ((key != null) && !key.equals("")
						&& key.startsWith(APPENDER_PREFFIX)) {
					key = key.trim();
					...
					if ((value != null) && !value.equals("")) {
						...
						if ((values != null) && (values.length > 1)) {
							...
							if (appenderType.equals(Appender.CONSOLE)) {
								appenders.put(appenderName, createAppender(
										appenderName, Appender.CONSOLE,
										appenderThreshold, appenderDest));

							} else if (appenderType.equals(Appender.TEXT_FILE)) {
								if ((appenderDest == null)
										|| appenderDest.equals("")) {
									appenderDest = DEFAULT_TEXT_LOG_FILENAME;
								}
								appenders.put(appenderName, createAppender(
										appenderName, Appender.TEXT_FILE,
										appenderThreshold, appenderDest));

							} else if (appenderType
									.equals(Appender.RICH_TEXT_FILE)) {
								if ((appenderDest == null)
										|| appenderDest.equals("")) {
									appenderDest = DEFAULT_RICHTEXT_LOG_FILENAME;
								}
								appenders.put(appenderName, createAppender(
										appenderName, Appender.RICH_TEXT_FILE,
										appenderThreshold, appenderDest));

							} else if (appenderType.equals(Appender.SCREEN)) {
								appenders.put(appenderName, createAppender(
										appenderName, Appender.SCREEN,
										appenderThreshold, appenderDest));
								...
								if ((appenderDest != null)
										&& !appenderDest.equals("")) {
									appenders.put(appenderName, createAppender(
											appenderName, Appender.EVENT_LOG,
											appenderThreshold, appenderDest));
								}
							}
						}
	...
	private static Appender createAppender(String name, String type,
			String pThreshold, String destination) {
